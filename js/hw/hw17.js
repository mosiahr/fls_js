...
class Product {
  ...
  constructor({ title, amount, unit, companyName, companyLicenceNumber }) {
    if (!title || !amount || !unit || !companyName || !companyLicenceNumber) {
...
class Warehouse {
        ...
        if (equalProductArr.length === 0) {
          for (const prod of this._products) {
            if (newProduct.company.equalsByName(prod.company)) {
              newProduct.company.licenceNumber = prod.company.licenceNumber
            }
          }
          this._products.push(newProduct)
        } else {
          for (const equalsProduct of equalProductArr) {
            this._products[equalsProduct[0]].addAmount(newProduct.amount)
          }
        }
      }
    } catch (error) {
  ...
  filterByProductTitle(productTitle) {
    try {
      return this._products.filter((product) => product._title === productTitle)
    } catch (error) {
      console.log(error)
    }
  }
  ...
  toString() {
    try {
      ...
      if (this._products && this._products.length !== 0) {
        const prodHeaders = Object.getOwnPropertyNames(this._products[0])
        ...
        headers.forEach((el, i, arrRef) => (arrRef[i] = el.replace('_', '')))
      } else {
        headers = []
      }
...
class TestData {
  ...
  set userAnswer(value) {
    this.#userAnswer = parseInt(value)
  }
  ...
  #addCorrectAnswer() {
    if (this.operation === '*') {
      this.#correctAnswer = MultChecker.multiply(
        this.#firstNum,
        this.#secondNum
      )
    } else if (this.operation === '+') {
      this.#correctAnswer = AddChecker.add(this.#firstNum, this.#secondNum)
  ...
  toString() {
    try {
      const res = this.#tasks.map((task, ind) => {
        ...
        const checkEl =
          task.userAnswer === task.correctAnswer ? check('green') : check('red')

        return [ind + 1, taskStr, userAnswerStr, task.correctAnswer, checkEl]
      })
...
class TestManager {
  ...
  run() {
    setTimeout(
      () =>
        this.createNotice(this.NOTICE_START_TESTS.toUpperCase(), 'test-start'),
      500
    )
